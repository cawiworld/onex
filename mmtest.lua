-- [[ oNex Hub v1.1 | Murder Mystery 2 ]] --
...
local Settings = {
    -- Тэбы и интерфейс
    CurrentTab = "General",
    MenuKeybind = Enum.KeyCode.RightControl, -- По умолчанию Right Control
    Visible = true,
...
-- Ограничитель скругления для боковой панели (чтобы правые углы не скруглялись внутри меню)
...
Logo.RichText = true -- Позволяет использовать HTML теги для цветов
...
local function SwitchTab(tabName, button)
    ...
    -- Сброс цвета остальных кнопок и подсветка выбранной
...
local function CreateTabButton(name)
    ...
    Label.Parent = Button
    Button:setAttribute("TextLabel", Label) -- для легкого доступа
...
--- [[[ КАСТОМНЫЕ ЭЛЕМЕНТЫ ИНТЕРФЕЙСА (Toggle, Slider, Dropdown) ]]] ---

-- 1. Создание анимированного Переключателя (Toggle)
...
-- 2. Создание анимированного Ползунка (Slider - Заготовка)
local function CreateSlider(parent, text, min, max, default)
    ...
    -- Логика перетаскивания (Тестовая рабочая механика ползунка)
...
-- 3. Быстрая циклическая кнопка смены вариантов (Вместо тяжелого Дропдауна)
...
-- 1. Вкладка: GENERAL (Заготовки ползунков и галочек)
...
-- 2. Вкладка: ESP (Наши основные функции + Расширенные настройки)
...
-- Кастомизация Цветов (Быстрые пресеты)
...
local function CreateHighlight(player, color)
    ...
    if Settings.NamesESP then
        ...
        txt.Font = Settings.ESPFont -- Подвязка выбранного шрифта
        txt.TextSize = 14
        txt.Parent = bbg
        bbg.Parent = char
    else
